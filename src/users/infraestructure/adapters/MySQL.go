...
func (mysql *MySQL) GetAll() ([]*entities.User, error) {
    query := "SELECT user_id, name, lastname, password, role, deleted_at FROM users WHERE deleted_at IS NULL"
    ...
    for rows.Next() {
        user := entities.User{}
        err := rows.Scan(&user.Id, &user.Name, &user.LastName, &user.Password, &user.Role, &user.DeletedAt)
        if err != nil {
            fmt.Println(err)
            return nil, err
        }
...
func (mysql *MySQL) Delete(id int32) error {
    ...
    result, err := mysql.conn.ExecutePreparedQuery(query, time.Now(), id)
    if err != nil {
